...
import { googleLoginUrl } from "@/lib/qdog-server";
...
export function SiteHeader() {
  ...
  const navItems: { href: string; label: string; matchPrefix?: string }[] = [
    ...
    { href: localePath(locale, "/eggs"), label: locale === "zh" ? "宠物蛋" : "Pet eggs" },
    ...
    <header className="site-header">
      <div className="site-header__inner flex items-center justify-between gap-2">
        ...
        <nav className="flex min-w-0 items-center gap-1" aria-label="Primary navigation">
          ...
          <a
            className="site-nav-link inline-flex px-2 py-2 sm:px-3"
            href={googleLoginUrl(localePath(locale, "/eggs"))}
          >
            {locale === "zh" ? "Google 登录" : "Google sign in"}
          </a>
