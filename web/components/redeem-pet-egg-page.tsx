  ...
  return (
    <main className="redeem-page" data-domain="cyber">
      <section className="redeem-panel">
        <div className="redeem-copy"><span className="section-kicker">{zh ? "代码兑换" : "Code redemption"}</span><h1>{zh ? "领取一枚赛博蛋" : "Reveal a Cyber Egg"}</h1><p>{zh ? "输入每日发放的代码。兑换成功后，隐藏的生命编码会生成专属 SVG 蛋体并存入背包。生命形象不能装进 Codex。" : "Enter a daily code. Its sealed life code will generate a unique SVG Cyber Egg in your backpack. Cyber Life cannot be installed into Codex."}</p><Link href={localePath(locale, "/eggs")}>{zh ? "查看今日发放代码 →" : "Browse today’s codes →"}</Link></div>
        <div className="redeem-machine">
          {asset ? <><PetEggVisual className="redeem-result" genomeCode={asset.genome.code} traits={asset.traits} /><strong>{zh ? "兑换成功" : "Redeemed"}</strong><code>{asset.genome.code}</code><Link className="asset-button asset-button--primary" href={localePath(locale, "/account")}>{zh ? "放入背包并查看" : "View in backpack"}</Link></> : account ? (
            <form onSubmit={(event) => void redeem(event)}><label htmlFor="egg-code">{zh ? "赛博蛋代码" : "Cyber Egg code"}</label><input id="egg-code" value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} placeholder="QD-XXXX-XXXX-XXXX" autoComplete="off" spellCheck={false} /><button disabled={pending || !code} type="submit">{pending ? (zh ? "正在兑换…" : "Redeeming…") : (zh ? "兑换赛博蛋" : "Redeem Cyber Egg")}</button>{error ? <p role="alert">{error}</p> : null}</form>
