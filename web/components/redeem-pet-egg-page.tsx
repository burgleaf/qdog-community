  ...
  return (
    <main className="redeem-page">
      <section className="redeem-panel">
        <div className="redeem-copy"><span className="section-kicker">{zh ? "代码兑换" : "Code redemption"}</span><h1>{zh ? "孵化一枚宠物蛋" : "Reveal a pet egg"}</h1><p>{zh ? "输入每日发放的宠物蛋代码。兑换成功后，代码内隐藏的属性会生成专属 SVG 宠物蛋并存入背包。" : "Enter a daily pet egg code. Its hidden traits will generate a unique SVG egg in your backpack."}</p><Link href={localePath(locale, "/eggs")}>{zh ? "查看今日发放代码 →" : "Browse today’s codes →"}</Link></div>
        <div className="redeem-machine">
          {asset ? <><PetEggVisual className="redeem-result" traits={asset.traits} /><strong>{zh ? "兑换成功" : "Redeemed"}</strong><code>{asset.code}</code><Link className="asset-button asset-button--primary" href={localePath(locale, "/account")}>{zh ? "放入背包并查看" : "View in backpack"}</Link></> : account ? (
            <form onSubmit={(event) => void redeem(event)}><label htmlFor="egg-code">{zh ? "宠物蛋代码" : "Pet egg code"}</label><input id="egg-code" value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} placeholder="QD-XXXX-XXXX-XXXX" autoComplete="off" spellCheck={false} /><button disabled={pending || !code} type="submit">{pending ? (zh ? "正在兑换…" : "Redeeming…") : (zh ? "兑换宠物蛋" : "Redeem pet egg")}</button>{error ? <p role="alert">{error}</p> : null}</form>
