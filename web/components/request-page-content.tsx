...
const requestIssueUrl =
  "https://github.com/burgleaf/qdog-community/issues/new?template=pet-request.yml";
...
const content = {
  en: {
    ...
    promiseBody:
      "QDog is an open community gallery. The project does not charge a request fee, and community-made pets are published with creator, source, and usage information. A request can be declined, remain unclaimed, or need better references.",
    ...
    faq: [
      {
        ...
        answer:
          "No. Opening a request in QDog is free. The community may create it voluntarily; the project does not promise a completion date or acceptance.",
      },
      {
  ...
  zh: {
    ...
    intro:
      "填写角色名称和公开图片链接后，会跳转到已预填内容的 GitHub Issue 表单，由你确认并发布。申请免费，社区制作由贡献者自愿完成。",
    codexCta: "使用 Codex 提交",
    manualCta: "填写后提交 GitHub Issue",
    githubCta: "去 GitHub 提交 Issue",
    plazaCta: "查看已有制作请求",
    note: "在线表单不会调用 QDog 提交接口；填写内容只会预填到 GitHub Issue，由你登录并确认发布。",
    howEyebrow: "申请流程",
    ...
    promiseBody:
      "QDog 是开放社区画廊，不收取制作申请费用。社区完成的宠物会公开作者、来源与使用说明；申请也可能暂时无人认领、被拒绝，或需要补充更清楚的参考资料。",
    afterTitle: "宠物收录之后",
    afterBody:
    ...
    faq: [
      {
        ...
        answer:
          "不收费。在 QDog 创建制作申请是免费的，社区可能志愿完成；项目不承诺一定收录，也不承诺完成时间。",
      },
      {
        question: "提交时需要什么？",
        answer:
  ...
  ko: {
    ...
    promiseBody:
      "QDog은 열린 커뮤니티 갤러리입니다. 요청 비용은 없으며 제작자, 출처, 사용 조건을 공개합니다. 요청은 미완료 상태로 남거나 추가 자료가 필요할 수 있습니다.",
    afterTitle: "펫이 공개되면",
    afterBody:
  ...
  ja: {
    ...
    promiseBody:
      "QDog はオープンなコミュニティギャラリーです。料金はなく、作者、出典、利用条件を公開します。未着手のまま残る場合や追加資料が必要な場合もあります。",
  ...
  es: {
    ...
    promiseBody:
      "QDog es una galería comunitaria abierta. No cobra por las peticiones y publica la autoría, fuente y condiciones de uso. Una petición puede quedar pendiente o necesitar mejores referencias.",
...
const directIssueCopy = {
  en: {
    intro:
      "Enter the character and a public image URL here, then review and publish the prefilled GitHub Issue. Requests are free and community production is voluntary.",
    manualCta: "Fill in a GitHub Issue request",
    note: "This form does not submit data to QDog. It only prefills a GitHub Issue for you to review and publish.",
    referenceStep:
      "The character name and a public HTTPS image URL are required. Add local image attachments directly in the GitHub Issue after continuing.",
    submitStep:
      "Use this form, Codex, or the GitHub Issue form. This form opens a prefilled Issue for you to confirm and publish.",
  },
  zh: {
    intro:
      "填写角色名称和公开图片链接后，会跳转到已预填内容的 GitHub Issue 表单，由你确认并发布。申请免费，社区制作由贡献者自愿完成。",
    ...
    note: "在线表单不会调用 QDog 提交接口；填写内容只会预填到 GitHub Issue，由你登录并确认发布。",
    referenceStep:
      "角色名称和公开 HTTPS 图片链接为必填。本地图片请在跳转后的 GitHub Issue 中直接添加附件。",
    submitStep:
      "可以填写此表单、使用 Codex，或直接打开 GitHub Issue；此表单会预填 Issue，由你确认并发布。",
  },
  ko: {
    intro:
      "캐릭터와 공개 이미지 URL을 입력하면 내용이 채워진 GitHub Issue 양식으로 이동합니다. 검토 후 직접 게시하며, 요청은 무료이고 제작은 자원봉사로 진행됩니다.",
    manualCta: "GitHub Issue 요청 작성",
    note: "이 양식은 QDog에 데이터를 제출하지 않으며, 검토하고 게시할 GitHub Issue만 미리 채웁니다.",
    referenceStep:
      "캐릭터 이름과 공개 HTTPS 이미지 URL이 필요합니다. 로컬 이미지는 이동 후 GitHub Issue에 직접 첨부하세요.",
    submitStep:
      "이 양식, Codex 또는 GitHub Issue 양식을 사용할 수 있습니다. 이 양식은 확인 후 게시할 Issue를 미리 채웁니다.",
  },
  ja: {
    intro:
      "キャラクターと公開画像 URL を入力すると、内容が入った GitHub Issue フォームに移動します。確認して自分で公開し、リクエストは無料で制作は有志によるものです。",
    manualCta: "GitHub Issue リクエストを入力",
    note: "このフォームは QDog にデータを送信せず、確認・公開する GitHub Issue を事前入力するだけです。",
    referenceStep:
      "キャラクター名と公開 HTTPS 画像 URL が必要です。ローカル画像は移動後に GitHub Issue へ直接添付してください。",
    submitStep:
      "このフォーム、Codex、または GitHub Issue フォームを使えます。このフォームは確認して公開する Issue を事前入力します。",
  },
  es: {
    intro:
      "Escribe el personaje y una URL pública de imagen; después revisarás y publicarás el GitHub Issue ya rellenado. La petición es gratuita y la creación es voluntaria.",
    manualCta: "Completar una petición de GitHub Issue",
    note: "Este formulario no envía datos a QDog; solo rellena un GitHub Issue para que lo revises y publiques.",
    referenceStep:
      "El nombre y una URL HTTPS pública de imagen son obligatorios. Adjunta imágenes locales directamente en GitHub después de continuar.",
    submitStep:
      "Puedes usar este formulario, Codex o el formulario de GitHub Issue. Este formulario rellena un Issue para que lo confirmes y publiques.",
  },
} as const;
...
export function RequestPageContent({
  ...
  const issueCopy = directIssueCopy[locale];
  ...
  return (
    ...
    >
      <header className="border-b border-border pb-12">
        ...
        <p className="mt-4 text-sm text-muted">{issueCopy.note}</p>
      </header>
          ...
          {copy.steps.map((step) => (
            ...
            >
              ...
              <p className="leading-7 text-text-secondary">
                {step.index === "01"
                  ? issueCopy.referenceStep
                  : step.index === "03"
                    ? issueCopy.submitStep
                    : step.description}
              </p>
            </li>
          ))}
