...
const pets = readJson("pets.json").map((pet) => {
  ...
  return {
    ...
    actions,
    gifs,
    installCommand: `curl -fsSL https://raw.githubusercontent.com/burgleaf/qdog-community/main/scripts/install-pet.sh | bash -s -- ${pet.slug}`,
    installCommandPowerShell: `powershell -NoProfile -ExecutionPolicy Bypass -Command "iwr -UseB https://raw.githubusercontent.com/burgleaf/qdog-community/main/scripts/install-pet.ps1 | iex; Install-CodexPet ${pet.slug}"`,
    repositoryPath: `https://github.com/burgleaf/qdog-community/tree/main/pets/${pet.slug}`,
  };
});
...
function completedPetForRequest(request) {
  ...
  const issueUrl = `https://github.com/burgleaf/qdog-community/issues/${request.number}`;
...
- [GitHub repository](https://github.com/burgleaf/qdog-community): Source, contribution history, and issue workflows.
