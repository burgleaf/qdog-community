...
const repository = process.env.GITHUB_REPOSITORY || "burgleaf/qdog-community";
const pageLimit = 5;
const pageSize = 100;
...
function reactionCount(issue) {
  ...
  return ["+1", "heart", "hooray", "rocket", "eyes"].reduce(
    (total, key) => total + (Number(reactions[key]) || 0),
    0,
  );
}
...
function toCatalogItem(issue) {
  ...
  const issueTitle = issue.title.replace(/^\s*\[request\]\s*:?\s*/i, "").trim();
...
const catalog = (await fetchIssues()).map(toCatalogItem).sort((left, right) => {
  const stateDelta =
