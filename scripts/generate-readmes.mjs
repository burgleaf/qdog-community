...
const rawBase =
  "https://raw.githubusercontent.com/burgleaf/qdog-community/main";
const websiteUrl = "https://q.dog";
...
function badges(pets) {
  return [
    ...
    "[![Pet previews](https://github.com/burgleaf/qdog-community/actions/workflows/pet-previews.yml/badge.svg)](https://github.com/burgleaf/qdog-community/actions/workflows/pet-previews.yml)",
  ].join(" ");
}
...
<h2><a href="${websiteUrl}">Browse and install free community Codex pets at QDog →</a></h2>
...
<a href="${websiteUrl}"><img src="./assets/cover/qdog-cover.png" alt="Open the QDog gallery"></a>

${badges(pets)}

</div>

This repository is the source catalog behind [QDog](${websiteUrl}): it keeps installable pet packages, creator attribution, collection metadata, validation tools, and contribution history. For browsing and installing pets, start with the website.

## Highlights

- **One-command install** — no clone, no manual setup, works on macOS / Linux / Windows
- **Free community gallery** — complete animation previews, collections, creator profiles, weekly rankings based on installs and likes, sharing, and community statistics at [QDog](${websiteUrl})
...
<h2><a href="${websiteUrl}">免费浏览并安装 Codex 小宠物：QDog →</a></h2>
...
<a href="${websiteUrl}"><img src="../../assets/cover/qdog-cover.png" alt="进入 QDog 精品画廊"></a>

${badges(pets)}

</div>

本仓库是 [QDog](${websiteUrl}) 背后的宠物目录，负责保存可安装成品、作者与来源信息、合集元数据、校验工具和贡献记录。挑选与安装宠物时，请优先使用网站。

## 亮点

- **一条命令安装** — 不需要克隆仓库，macOS / Linux / Windows 全平台支持
- **免费社区画廊** — [QDog](${websiteUrl}) 提供完整动作预览、合集、作者主页、基于安装与点赞的每周榜单、便捷分享和社区统计
...
<h2><a href="${websiteUrl}">QDog에서 무료 커뮤니티 Codex 펫을 둘러보고 설치하세요 →</a></h2>
...
<a href="${websiteUrl}"><img src="../../assets/cover/qdog-cover.png" alt="QDog 갤러리 열기"></a>

${badges(pets)}

</div>

이 저장소는 [QDog](${websiteUrl})의 원본 카탈로그입니다. 설치 가능한 펫 패키지, 제작자 정보, 컬렉션 메타데이터, 검증 도구, 기여 이력을 관리합니다. 펫을 둘러보고 설치하려면 웹사이트를 먼저 이용하세요.

## 주요 기능

- **한 줄 설치** — 저장소 복제나 수동 설정 없이 macOS / Linux / Windows에서 설치
- **무료 커뮤니티 갤러리** — [QDog](${websiteUrl})에서 완성된 애니메이션 미리 보기, 컬렉션, 제작자 프로필, 설치 수와 좋아요를 기준으로 한 주간 순위, 공유, 커뮤니티 통계 제공
...
const additionalReadmeCopy = {
  ja: {
    language: "日本語",
    siteHeading: "QDog で無料のコミュニティ Codex ペットを探してインストール →",
    intro:
      "QDog は、コミュニティが制作した無料の Codex ペットギャラリーです。アニメーションを確認し、リポジトリを複製せずにお気に入りをインストールできます。まだないキャラクターはコミュニティへ制作をリクエストできます。",
    browse: "ペットを見る",
    install: "インストール",
    request: "キャラクターをリクエスト",
    imageAlt: "QDog ギャラリーを開く",
    source:
      "このリポジトリは [QDog] のソースカタログです。インストール可能なペット、作者と出典、コレクション情報、検証ツール、貢献履歴を管理しています。",
    highlightsTitle: "特徴",
    highlights: [
  ...
  es: {
    language: "Español",
    siteHeading: "Explora e instala mascotas gratuitas de Codex en QDog →",
    intro:
      "QDog es una galería gratuita de mascotas creadas por la comunidad. Puedes revisar sus animaciones, instalar tus favoritas sin clonar el repositorio o pedir un personaje que todavía no exista.",
    browse: "Explorar mascotas",
    install: "Instalar una mascota",
    request: "Pedir un personaje",
    imageAlt: "Abrir la galería de QDog",
    source:
      "Este repositorio es el catálogo fuente de [QDog]. Conserva los paquetes instalables, la autoría y procedencia, las colecciones, las herramientas de validación y el historial de contribuciones.",
...
<a href="${websiteUrl}/${localePath}"><img src="../../assets/cover/qdog-cover.png" alt="${copy.imageAlt}"></a>

${badges(pets)}

</div>

${copy.source.replace("[QDog]", `[QDog](${websiteUrl}/${localePath})`)}
