...
已经完成且能够在 Codex 中运行的宠物，可以通过 `[Showcase]` Issue 向社区展示并征求反馈。每个 Issue 展示一只宠物，至少提供一张公开预览或图片附件，如实注明创作者与重要素材来源；还可以直接链接到 QDog 详情页、公开源码仓库、项目主页或安装说明。不接受推广返利链接、短链接或付费下载链接。

Showcase 不代表申请收录，不转移作品权利，也不能代替素材许可说明。需要把宠物新增或更新到 QDog 目录时，仍应创建 Ready for review 的正式 PR；只有正常投稿流程经过恢复后仍确实受阻，才使用 `[Submission]` Issue。
