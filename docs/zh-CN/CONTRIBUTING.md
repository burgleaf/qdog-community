...
一只或多只已经完成且能够在 Codex 中运行的宠物，可以合并到同一个 `[Showcase]` Issue 中向社区展示并征求反馈。公开预览或图片附件应清楚覆盖每只展示的宠物，并分别如实注明创作者与重要素材来源；还可以直接链接到对应的 QDog 详情页、公开源码仓库、项目主页或安装说明。不接受推广返利链接、短链接或付费下载链接。

Showcase 不代表申请收录，不转移作品权利，也不能代替素材许可说明。需要把这些宠物新增或更新到 QDog 目录时，仍应为每只宠物分别创建 Ready for review 的正式 PR；只有正常投稿流程经过恢复后仍确实受阻，才使用 `[Submission]` Issue。
